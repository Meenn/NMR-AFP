...
def train(config, training_chunked_samples_dir, testing_chunked_samples_file):
    ...
    ML_MemAE_SC_state_dict = torch.load(config["NL_ML_MemAE_pretrained"])["model_state_dict"]
    ...
    if not config["pretrained"]:
        model.vunet.apply(weights_init_kaiming)
    else:
        ...
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            for idx, train_data in tqdm(enumerate(dataloader),
                ...
                if step % config["logevery"] == config["logevery"] - 1:
                    ...
                    print(
                        "[loss_kl: {:.4f} loss_frame: {:.4f} loss_grad: {:.4f}]".
                            format(loss_kl, loss_frame, loss_grad))
    ...
    print(
        "================ Best AUC %.4f " % best_auc + "of epoch %d" % best_auc_epoch + " loss_frame: %.4f " % loss3 + " loss_grad: %.4f" % loss4 + "================")
