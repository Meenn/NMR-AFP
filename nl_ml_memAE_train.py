...
from models.NL_ML_MemAE import NL_ML_MemAE
...
def train(config, training_chunked_samples_dir, testing_chunked_samples_file):
    ...
    model = NL_ML_MemAE(num_in_ch=config["model_paras"]["motion_channels"],
    ...
    # copy config file
    shutil.copyfile("cfgs/nl_ml_memAE_cfg.yaml",
                    os.path.join(config["log_root"], config["exp_name"], "nl_ml_memAE_cfg.yaml"))
    ...
    for epoch in range(epoch_last, epochs + epoch_last):
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            #####传入flows，若改为frames，sample_frames, _, _, _, _
        ...
        if epoch % config["saveevery"] == config["saveevery"] - 1:
            ...
            # evaluation
            with torch.no_grad():
                auc = nl_ml_memAE_eval.evaluate(config, model_save_path + "-%d" % (epoch + 1),
...
if __name__ == '__main__':
    config = yaml.safe_load(open("cfgs/nl_ml_memAE_cfg.yaml"))
